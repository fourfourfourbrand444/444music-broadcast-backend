/**
 * templates/announcement.js
 *
 * General-purpose announcement template. Accepts optional
 * "title" and "message" fields via templateData for flexibility.
 * Exports a function: ({ user, title, message }) => ({ html, text })
 */

function announcement({ user, title, message }) {
  const headline = title || 'An Announcement from 444Music';
  const body = message || 'We have an update to share with you.';
        ...
        <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="background-color:#111111;border-radius:16px;overflow:hidden;border:1px solid #2a2a2a;">
          <tr>
            <td style="padding:32px;text-align:center;border-bottom:1px solid #2a2a2a;">
              <span style="display:inline-block;background-color:rgba(139,92,246,0.15);border:1px solid rgba(139,92,246,0.3);border-radius:100px;padding:6px 16px;color:#c4b5fd;font-size:11px;letter-spacing:1.5px;text-transform:uppercase;font-weight:600;">Announcement</span>
            </td>
          </tr>
          <tr>
            <td style="padding:40px 32px;color:#e5e5e5;">
              <h2 style="margin:0 0 16px;color:#ffffff;font-size:22px;">${headline}</h2>
              ...
              <table role="presentation" cellpadding="0" cellspacing="0">
                <tr>
                  <td style="background:linear-gradient(135deg,#5b3fd1,#8b5cf6);border-radius:100px;">
                    <a href="#" style="display:inline-block;padding:14px 32px;color:#ffffff;text-decoration:none;font-weight:600;font-size:14px;">Learn More</a>
                  </td>
                </tr>
              </table>
            </td>
